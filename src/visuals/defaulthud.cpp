...
void DefaultHud::render(Renderer &renderer, Gamestate &state, Player &myself)
{
    ...
    if (state.exists(myself.character))
    {
        ...
        if (myself.ultcharge.active)
        {
            ...
            Rect ultbarrect = renderer.spriteloader.get_rect("ui/ingame/ultbar").offset(renderer.WINDOW_WIDTH/2,
                                                                                        HUD_Y_BASELINE);
            al_draw_bitmap(ultbar, ultbarrect.x, ultbarrect.y, 0);
            al_draw_arc(ultbarrect.x+ultbarrect.w/2.0, ultbarrect.y+ultbarrect.h/2.0 - 8, 33*renderer.zoom,
                        -3.1415/2.0, 2*3.1415*myself.ultcharge.timer/100.0, al_map_rgb(255, 230, 125), 8);
        }
        else
        {
            ALLEGRO_BITMAP *ultbar = renderer.spriteloader.requestsprite("ui/ingame/"+character.herofolder()
                                                                         + "ultready");
            Rect ultbarrect = renderer.spriteloader.get_rect("ui/ingame/"+character.herofolder()
                                                             + "ultready").offset(renderer.WINDOW_WIDTH/2,
                                                                                  HUD_Y_BASELINE);
            al_draw_bitmap(ultbar, ultbarrect.x, ultbarrect.y, 0);
        }
    }
